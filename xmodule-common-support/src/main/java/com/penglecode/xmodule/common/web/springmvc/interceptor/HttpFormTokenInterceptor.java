package com.penglecode.xmodule.common.web.springmvc.interceptor;
...
import com.penglecode.xmodule.common.web.support.DefaultHttpFormTokenValueDAO;
import com.penglecode.xmodule.common.web.support.HttpFormToken;
import com.penglecode.xmodule.common.web.support.HttpFormTokenValueDAO;
import com.penglecode.xmodule.common.web.support.HttpResubmitException;
 ...
 * @version  	1.0
 */
public class HttpFormTokenInterceptor extends HandlerInterceptorAdapter implements ApplicationContextAware, InitializingBean {

	private static final Logger logger = LoggerFactory.getLogger(HttpFormTokenInterceptor.class);
	...
	private HttpFormTokenValueDAO tokenValueDAO = new DefaultHttpFormTokenValueDAO();
	...
	protected HttpFormTokenValueDAO getTokenValueDAO() {
		return tokenValueDAO;
	}

	public void setTokenValueDAO(HttpFormTokenValueDAO tokenValueDAO) {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		if(handler instanceof HandlerMethod){
			...
			HttpFormToken token = method.getAnnotation(HttpFormToken.class);
			if(token != null){
				String[] keys = token.key();
				if(token.action() == HttpFormToken.TokenAction.CREATE){
				...
				if(token.action() == HttpFormToken.TokenAction.VALIDATE){
					if(keys != null && keys.length > 0){
						for(String key : keys){
	...
	protected void handleRepeatSubmit(HttpServletRequest request, HttpServletResponse response, Object handler){
		throw new HttpResubmitException(getErrorMessage(request));
	}
