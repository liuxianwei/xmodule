...
import com.penglecode.xmodule.springsecurity.upms.consts.em.UpmsResourceActionTypeEnum;
import com.penglecode.xmodule.springsecurity.upms.consts.em.UpmsResourceTypeEnum;
import com.penglecode.xmodule.springsecurity.upms.mapper.UpmsResourceMapper;
import com.penglecode.xmodule.springsecurity.upms.model.UpmsResource;
import com.penglecode.xmodule.springsecurity.upms.service.UpmsResourceService;

@Service("upmsResourceService")
public class UpmsResourceServiceImpl implements UpmsResourceService, RoleResourceService {

	public static final Consumer<UpmsResource> UPMS_RESOURCE_DECODER = model -> {
		if(model.getResourceType() != null){
			UpmsResourceTypeEnum em = UpmsResourceTypeEnum.getType(model.getResourceType());
			if(em != null){
				model.setResourceTypeName(em.getTypeName());
			}
		}
		if(model.getActionType() != null){
			UpmsResourceActionTypeEnum em = UpmsResourceActionTypeEnum.getType(model.getActionType());
			if(em != null){
	...
	public void deleteResourceById(Long resourceId, boolean cascadeDelete) {
		...
		if(cascadeDelete){
			List<UpmsResource> allMyAppResourceList = upmsResourceMapper.selectResourceListByParam(null, Boolean.TRUE);
			if(!CollectionUtils.isEmpty(allMyAppResourceList)){
				...
				for(int i = 0, len = childResourceList.size(); i < len; i++){
					upmsResourceMapper.deleteModelById(childResourceList.get(i).getResourceId());
				}
			}
		}else{
	...
	public UpmsResource getResourceById(Long resourceId) {
		return ModelDecodeUtils.decodeModel(upmsResourceMapper.selectModelById(resourceId), UPMS_RESOURCE_DECODER);
	}
	...
	public List<UpmsResource> getResourceListByParentId(Long parentResourceId, Integer actionType) {
		...
		return ModelDecodeUtils.decodeModel(upmsResourceMapper.selectModelListByExample(example, Sort.by(Order.asc("siblingsIndex"))), UPMS_RESOURCE_DECODER);
	}

	@Override
	public List<UpmsResource> getResourceListByPage(UpmsResource condition, Page page, Sort sort) {
		List<UpmsResource> dataList = ModelDecodeUtils.decodeModel(upmsResourceMapper.selectModelPageListByExample(condition, sort, new RowBounds(page.getOffset(), page.getLimit())), UPMS_RESOURCE_DECODER);
	...
	public List<UpmsResource> getResourceListByParam(Integer actionType, boolean fetchInuse) {
		return ModelDecodeUtils.decodeModel(upmsResourceMapper.selectResourceListByParam(actionType, fetchInuse), UPMS_RESOURCE_DECODER);
	}
