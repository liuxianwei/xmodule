 ...
 */  
APP_UPMS_API = {
	CONTEXT_PATH: '/flexedgex-upms',
	APP_DESCRIPTION: '统一用户权限管理系统后台服务'
};
APP_UPMS_WEB = {
	CONTEXT_PATH: '/flexedgex-upms-web',
 ...
 */
var ASIDE_MENU_COLLAPSE_STATE_KEY = "ASIDE_MENU_COLLAPSE";
/**
 * 浏览器窗口高度,宽度
 */
...
var AdminContainer = Vue.extend({
	template: '<el-container :style="{height: windowHeight}">'
				+ '<el-header :height="headerHeight">'
					+ '<div class="el-row admin-header" :class="{collapsed: aisdeMenuCollapse}">'
						+ '<h3 class="el-col admin-logo"><a href="javascript:;"><img :src="logoUrl"/></a><label>{{appTitle}}</label></h3>'
						+ '<nav class="el-col el-row admin-navbar">'
							+ '<div class="el-col nav-menu">'
								...
								+ '<admin-topnav-menu :default-active="defaultTopnavMenuActive" :menu-list="topnavMenuList" :menu-class="topnavMenuClass"></admin-aside-menu>'
							+ '</div>'
							+ '<div class="el-col tool-menu">'
								+ '<el-menu class="admin-tool-menu" mode="horizontal">'
												...
												+ '<el-dropdown-item>'
													+ '<div class="admin-dropdownpanel-userinfo">'
														...
														+ '<div class="el-row" style="margin-top:10px;">'
															+ '<div class="el-col el-col-8 actions"><router-link class="el-button el-button--default profile" tag="button" to="/admin/user/profile.html"><span>资料</span></router-link></div>'
															+ '<div class="el-col el-col-8 actions"><router-link class="el-button el-button--default settings" tag="button" to="/admin/user/settings.html"><span>设置</span></router-link></div>'
															+ '<div class="el-col el-col-8 actions"><router-link class="el-button el-button--default logout" tag="button" to="/logout"><span>退出</span></router-link></div>'
														+ '</div>'
													+ '</div>'
												+ '</el-dropdown-item>'
											+ '</el-dropdown-menu>'
									...
									+ '<el-menu-item index="/admin/logout">'
										+ '<router-link to="/logout"><i class="el-icon-fa-power-off" title="退出"></i></router-link>'
									+ '</el-menu-item>'
								+ '</el-menu>'
							+ '</div>'
						+ '</nav>'
				...
				+ '<el-container>'
					+ '<el-aside v-if="showAside" class="admin-aside" :width="asideWidth">'
						+ '<div class="side-menu">'
							...
							+ '</div>'
							+ '<admin-aside-menu :default-active="defaultAsideMenuActive" :menu-list="asideMenuList" :menu-class="asideMenuClass" :collapse="aisdeMenuCollapse" @select="onAsideMenuSelected"></admin-aside-menu>'
						+ '</div>'
					+ '</el-aside>'
	...
	created: function(){
		this.currentAsideMenuActive = this.defaultAsideMenuActive;
...
var AdminRouter = new VueRouter({
	mode: 'history',
	routes: [{
		path: '/user/profile.html', //用户资料
		meta: {
			menuUrl: '/user/profile.html'
		},
		beforeEnter: goToUrl
	},{
		path: '/user/settings.html', //用户设置
		meta: {
			menuUrl: '/user/settings.html'
		},
		beforeEnter: goToUrl
	},{
		path: '/logout', //用户退出
	...
	methods: {
		applyDefaultActiveMenu: function(){
			...
			if(currentRoute && currentRoute.path){
				...
				if(start > 0){
					var defaultAvtive = path.substring(start);
					this.defaultAsideMenuActive = defaultAvtive;
					this.defaultTopnavMenuActive = defaultAvtive;
				}
			}
		},
		/**
			...
			axios.get(APP_UPMS_API.CONTEXT_PATH + '/login/user/allinfo').then(function(response){
				if(response.data){
					...
					if(result && result.success){
						...
						var loginUserMenuList = allInfo.loginUserMenus; //当前登录用户菜单树
						var routes = [];
						loginUserMenuList = _this.prepareUserMenuList(loginUserMenuList);
						_this.recurisiveLoadRouters(loginUserMenuList, routes);
						_this.loginUserMenuList = loginUserMenuList;
		 ...
		 * 预处理后台加载过来的用户菜单列表
		 */
		prepareUserMenuList: function(userMenuList){
			var menuList = [];
			if(userMenuList && userMenuList.length){
				...
				//向其菜单集合的第一个位置插入首页菜单
				menuList.unshift(this.defaultIndexMenu);
			}
			return menuList;
		},
		/**
		 * 递归动态加载路由
		 */
		recurisiveLoadRouters: function(menuList, routes){
			if(menuList && menuList.length){
				var _this = this;
				menuList.forEach(function(node){
					if(node.menuUrl){
						node.menuPath = node.menuUrl;
					}
					var route = {
						path: node.menuPath,
						meta: {
					...
					routes.push(route);
					if(node.subMenuList){
						_this.recurisiveLoadRouters(node.subMenuList, routes);
					}
				});
			}
		}
